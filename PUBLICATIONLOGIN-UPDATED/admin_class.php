...
Class Action {
	...
	function save_upload(){
		...
		// If there's no ID, insert a new record
		if(empty($id)){
			$save = $this->db->query("INSERT INTO documents SET $data, status='PENDING'");
		} else {
			// If there's an ID, update the existing record
			$save = $this->db->query("UPDATE documents SET $data, status='PENDING' WHERE id = $id");
		}
